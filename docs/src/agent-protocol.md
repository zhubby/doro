...
The stream also carries direct control-plane commands for connected agents. Container refresh uses `CollectContainersCommand`. Terminal execution uses `RunTerminalCommandCommand`: the control plane sends a one-line shell command to an online agent that declares `ShellExecute`, the agent writes it to its persistent local PTY session, and the agent replies with `TerminalCommandResultEvent` containing output, exit code, and start/finish timestamps. The UI terminal route currently exposes this as a request/response command flow rather than a fully interactive byte stream.
