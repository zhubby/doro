...
`doro-config` owns TOML configuration loading and default config creation for `~/.doro/control-plane.toml` and `~/.doro/agent.toml`, including Agent reliability settings for event spooling, cancellation grace, and preflight behavior.
