...
Docker container creation is also an explicit operator-selected direct-execution path. Direct creation still creates a task, task step, task run, and Docker command event before dispatching over the enrolled Agent stream, and it still requires an online Agent that declares `ContainersManage`. Operators can switch the create request to the approval path when local change-control policy requires a pending approval before dispatch. Docker image pull, Compose create/update, Compose pull, and Compose up are audited direct-execution paths so routine deployment refreshes do not wait on approval. Docker registry credential management updates the target Agent user's default Docker config and is audited without storing registry secrets in control-plane task payloads. Docker lifecycle, image removal, network, volume, and remaining Compose actions continue to use the approval workflow by default.
