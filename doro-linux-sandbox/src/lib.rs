...
/// binaries (`codex`, `doro-exec`, `doro-tui`) around dispatching to the
/// `doro-linux-sandbox` sub-command.
///
/// When the current executable is invoked through the hard-link or alias
/// named `doro-linux-sandbox` we *directly* execute [`run_main`](crate::run_main)
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    ...
    if exe_name == "doro-linux-sandbox" {
...
pub fn run_main() -> ! {
    panic!("doro-linux-sandbox is only supported on Linux");
}
